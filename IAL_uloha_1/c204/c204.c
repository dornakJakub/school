...
char *infix2postfix( const char *infixExpression ) {
	...
	//Alokace pameti pro postfixovy retezec
	char *postfixExpression = (char *)malloc(MAX_LEN * sizeof(char));
	
	//Kontrola spravne alokace pameti
	...
	while (infixExpression[i] != '=') {
		...
		//If statement rozhoduje o dalsim postupu prevodu dle aktualniho znaku, viz. studijni opora str. 61
...
void expr_value_push( Stack *stack, int value ) {
	...
	Stack_Push(stack, (value >> 24) & 0xFF);
	Stack_Push(stack, (value >> 16) & 0xFF);
	Stack_Push(stack, (value >> 8) & 0xFF);
	Stack_Push(stack, value & 0xFF);
...
bool eval( const char *infixExpression, VariableValue variableValues[], int variableValueCount, int *value ) {
	char currChar, *postfixExpression = infix2postfix(infixExpression);
	unsigned i = 0;
	Stack stack;
	Stack_Init(&stack);
	int counter = 0;
	...
	while (postfixExpression[i] != '\0') {
		currChar = postfixExpression[i];
		//If statement rozhoduje o dalsim postupu vyhodnoceni dle aktualniho znaku, viz. studijni opora str. 60
		if (currChar >= '0' && currChar <= '9') {
			//Konverze charu na int, ktery je pote ulozen na zasobnik pomoci funkce expr_value_push
			int intValue = currChar - '0';
			expr_value_push(&stack, intValue);
		}
		else if ((currChar >= 'a' && currChar <= 'z') || (currChar >= 'A' && currChar <= 'Z')) {
			//Vyhleda hodnotu promenne v poli variableValues, hodnota je pote ulozena na zasobnik pomoci funkce expr_value_push
			for (int j = 0; j < variableValueCount; j++) {
				if (variableValues[j].name == currChar) {
			...
			//Vyhodnoceni operatoru. Vzhledem k tomu ze jsou vsechny dyadicke, potrebuju vzdy dva operandy 
			int a, b, result;
